...
src/        library modules (hdc.py, encoder.py, hdc_train.py, nystrom_encoder.py,
            ...
            pynn_lif_model.py, generate_spinnaker_script.py) and src/data/ (small
            committed artifacts, e.g. exported model weights)
...
notebooks/  exploratory prototypes (hdc_lab.ipynb, hdc_nystrom.ipynb) and EBRAINS batch-
            submission notebooks (spinnaker_lif_shd_deploy.ipynb) -- each notebook's first
            code cell chdirs to the repo root and adds src/ to sys.path
verification/  correctness checks for the SpiNNaker port, run before spending hardware
                queue time -- see verification/README.md for the tier breakdown
...
A different kind of comparison from the ones above: not another HDC/SNN accuracy sweep on
UWaveGestureLibrary, but a hardware-profiling exercise — deploying a **trained `sparch` LIF
model** (a separate PyTorch SNN codebase, not part of this repo) on the **Spiking Heidelberg
Digits (SHD)** dataset to real **SpiNNaker neuromorphic silicon**, via PyNN and the EBRAINS
Neuromorphic Computing Platform's batch-submission API (`nmpi`).

**Why LIF, not adLIF**: `sparch` also has an adaptive-LIF (adLIF) variant with a
subthreshold-voltage-coupling term that has no equivalent in standard PyNN neuron models — plain
LIF maps directly onto PyNN's `IF_curr_exp` (current-based synapses, matching sparch's
current-like, non-conductance-based formulation), so it's the lower-risk model to port first.

**Why SHD, not HD**: SHD's input is already a spike train (precomputed spike times, binned by
`sparch`'s own dataloader into 100 steps over 1.4s — an exact, known 14ms/step), so every layer,
including the input layer, can be a genuine PyNN `Population` connected by `Projection`s. HD's
input is continuous mel-filterbank features that would need a host-side current-injection
workaround for the first layer alone.

## The pipeline

```
sparch (separate repo)                    hdc-language (this repo)
─────────────────────                     ─────────────────────────
...
**1-2. Train + export** (in the `sparch` repo, not this one):
...
cp shd_lif_weights.npz /path/to/hdc-language/src/data/
```

A longer 50-epoch training run is available as a TACC batch job:
`sbatch slurm/train_lif_shd.slurm` (in the `sparch` repo, CPU queue — this training loop has no
GPU code, ~27s/epoch observed locally).
...
**Is this a faithful hardware deployment?** Worth being honest about, matching this repo's
existing habit of self-critical write-ups: every layer is a genuine spiking `Population`
connected by `Projection`s (not a precomputed-current-replay workaround), which is the
architecturally meaningful choice for profiling real event-driven neuromorphic hardware — but
getting there required an empirically-derived current-scaling correction, not a clean analytic
translation, and the two approximations above mean predictions will diverge from sparch's exact
trained behavior on some inputs. The right way to think about SpiNNaker's output here is "a
plausible, input-driven approximation of the trained model, deployed in a way that's meaningful
for hardware profiling" — not "the trained model, verbatim, on different silicon."
